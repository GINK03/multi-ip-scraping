...
- スクレイピングしていると、どうしても時間あたりの1ドメインにつき、1スクレイピングを担保しながら、量を最大化しないと業務効率が伸び悩むことがある
...
今回は、AWSのスポットインスタンスや、GCPのプリエンプティプインスタンスを利用することで、ブロックされてもリスクのないIPを取得することがきますので、プロキシサーバにして、利用することでGlobalIPなどを汚染しないで済みます  
...
つまり、multiprocessingなどで並列でアクセスしても、**一プロセス1ドメインでsleepを入れて1req/secになる様に調整する必要**があります  
